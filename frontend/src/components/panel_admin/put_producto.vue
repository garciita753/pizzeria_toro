...
const categoriaLabel = computed(() =>
  CAT_LABELS[form.value.categoriaId] ?? 'Desconocida'
)
...
function validate(): boolean {
  ...
  if (esPizza.value) {
    const tieneAlMenosUno = Object.values(form.value.preciosTamano).some(p => Number(p) > 0)
    if (!tieneAlMenosUno) {
...
const handleSubmit = async () => {
  if (!validate() || !props.productoId) return
